...
using System.Linq;
using System.Text;
using ReadOnlyLogicalCellsMap = System.Collections.Generic.IReadOnlyDictionary<
    RCi.Toolbox.Ptt.Coordinate /* logical coordinate */
    ,
    RCi.Toolbox.Ptt.LogicalCell
>;
using ReadOnlyLogicalToPhysicalMap = System.Collections.Generic.IReadOnlyDictionary<
    RCi.Toolbox.Ptt.Coordinate /* logical coordinate */
    ,
...
namespace RCi.Toolbox.Ptt
{
    internal static class TableRenderer
    {
        ...
        {
            var sumGlobalBordersWidth =
                physicalVerticalBorderWidths.Length == 0 ? 0 : physicalVerticalBorderWidths.Sum();
            var sumGlobalBordersHeight =
                physicalHorizontalBorderHeights.Length == 0 ? 0 : physicalHorizontalBorderHeights.Sum();
            ...
            var canvasSize = new Size(
                sumGlobalBordersWidth + sumGlobalCellsWidth,
                sumGlobalBordersHeight + sumGlobalCellsHeight
            );
            var canvas /* [row, col] */
            = Enumerable.Range(0, canvasSize.Height).Select(_ => new char[canvasSize.Width]).ToArray();
            var borderMask /* [row, col] */
            = Enumerable.Range(0, canvasSize.Height).Select(_ => new Border[canvasSize.Width]).ToArray();
        ...
        )
        {
            ...
            PaintBorder(
                ...
                borderMask,
                margins.LeftBorder,
                GetVerticalBorderCharacter(style, logicalCell.Borders.Left),
                logicalCell.Borders.Left
            );
            PaintBorder(
                canvas,
                borderMask,
                margins.TopBorder,
                GetHorizontalBorderCharacter(style, logicalCell.Borders.Top),
                logicalCell.Borders.Top
            );
            PaintBorder(
                canvas,
                borderMask,
                margins.RightBorder,
                GetVerticalBorderCharacter(style, logicalCell.Borders.Right),
                logicalCell.Borders.Right
            );
            PaintBorder(
                canvas,
                borderMask,
                margins.BottomBorder,
                GetHorizontalBorderCharacter(style, logicalCell.Borders.Bottom),
                logicalCell.Borders.Bottom
            );
            ...
            PaintBorderCorner(
                canvas,
                borderMask,
                margins.TopLeftBorderCorner,
                GetBorderCornerCharacter(style, topLeftBorder),
                topLeftBorder
            );
            PaintBorderCorner(
                canvas,
                borderMask,
                margins.TopRightBorderCorner,
                GetBorderCornerCharacter(style, topRightBorder),
                topRightBorder
            );
            PaintBorderCorner(
                canvas,
                borderMask,
                margins.BottomRightBorderCorner,
                GetBorderCornerCharacter(style, bottomRightBorder),
                bottomRightBorder
            );
            PaintBorderCorner(
                canvas,
                borderMask,
                margins.BottomLeftBorderCorner,
                GetBorderCornerCharacter(style, bottomLeftBorder),
                bottomLeftBorder
            );
        }
        ...
        )
        {
            ...
            var topLeftPhysicalCell = physicalCells.OrderBy(x => x).First();
            var bottomRightPhysicalCell = physicalCells.OrderByDescending(x => x).First();
            ...
            var borderTopRight = new Margin(
                area.Right,
                area.Top - topBorderSize,
                area.Right + rightBorderSize,
                area.Top
            );
            var borderRight = area with { Left = area.Right, Right = area.Right + rightBorderSize };
            var borderBottomRight = new Margin(
                area.Right,
                area.Bottom,
                area.Right + rightBorderSize,
                area.Bottom + bottomBorderSize
            );
            var borderBottom = area with { Top = area.Bottom, Bottom = area.Bottom + bottomBorderSize };
            var borderBottomLeft = new Margin(
                area.Left - leftBorderSize,
                area.Bottom,
                area.Left,
                area.Bottom + bottomBorderSize
            );
        ...
        private static void PaintBorderCorner(
            char[][] canvas,
            Border[][] borderMask,
            Margin rectangle,
            char color,
        ...
        private static char GetHorizontalBorderCharacter(BorderStyle style, Border border) =>
            style switch
            {
        ...
        private static char GetVerticalBorderCharacter(BorderStyle style, Border border) =>
            style switch
            {
                BorderStyle.Ascii => border switch
                {
        ...
        private static char GetBorderCornerCharacter(BorderStyle style, Border border) =>
            style switch
            {
                BorderStyle.Ascii => border switch
                {
        ...
        private static char ReconcileSingle(Border left, Border top, Border right, Border bottom) =>
            left switch
            {
        ...
        private static char ReconcileDouble(Border left, Border top, Border right, Border bottom) =>
            left switch
            {
                Border.None => top switch
                {
